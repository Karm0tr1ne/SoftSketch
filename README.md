# SoftSketch

This repo is for the HKU COMP7506 Smartphone Apps Development Group Project. This project is a game demo which uses the Soft2D plugin developed by Taichi to simulate the motion of fluids in 2D space.



## Environment Requirements

| Unity Version           | Graphics API    | Rendering Pipeline | Scripting Backend |
| ----------------------- | --------------- | ------------------ | ----------------- |
| 2021.3.22f1c1 or higher | Vulkan or Metal | Built-in or URP    | IL2CPP            |

> Note: MacOS currently only supports M1 chips.



## References

- [Soft2D for Unity](https://github.com/taichi-dev/soft2d-for-unity)

- [Soft2D](https://github.com/taichi-dev/soft2d-release)